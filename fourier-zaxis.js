(function(){
console.log('📐 Z轴模块 v29.0 稳定修复版｜参考你的显示方式');
...
// 【修复】加权概率计算：基于价格斜率 + 频谱方向
function computeWeightedProb(fullSpectrum, prices){
    if(!fullSpectrum || fullSpectrum.length===0) return {up:50, down:25, flat:25};
    
    // 【修复核心】方法1：基于实际价格斜率（5日 + 20日）
    var priceSlope = 0;
    if(prices && prices.length >= 5) {
        // 5日斜率（短期）
        var last5 = prices.slice(-5);
        var slope5 = (last5[last5.length-1] - last5[0]) / last5[0];
        
        // 20日斜率（中期）
        var slope20 = 0;
        if(prices.length >= 20) {
            var last20 = prices.slice(-20);
            slope20 = (last20[last20.length-1] - last20[0]) / last20[0];
            // 综合：60%短期 + 40%中期
            priceSlope = slope5 * 0.6 + slope20 * 0.4;
        } else {
            priceSlope = slope5;
        }
    }
    // 斜率归一化：5%变化 = 方向1或-1
    var normalizedSlope = Math.max(-1, Math.min(1, priceSlope / 0.05));
    ...
    // 70% 基于实际价格斜率
    // 30% 基于频谱方向
    var finalDir = normalizedSlope * 0.7 + weightedDir * 0.3;
    finalDir = Math.max(-1, Math.min(1, finalDir));
    
    // 转换为概率（范围扩大：8%-92%）
    var upProb = 50 + finalDir * 40;
    upProb = Math.min(92, Math.max(8, upProb));
    var downProb = Math.max(3, Math.min(85, 100 - upProb - 20));
    var flatProb = 100 - upProb - downProb;
    
    console.log('方向计算 | 斜率:', (normalizedSlope*100).toFixed(1) + '% | 频谱:', (weightedDir*100).toFixed(1) + '% | 最终:', (finalDir*100).toFixed(1) + '%');
...
function refreshAll(){
    try{
        ...
        // 【修复】传入prices参数
        prob = computeWeightedProb(fullSpectrum, prices);
        updatePanel();
...
function updatePanel(){
    ...
    panel.innerHTML='<div><div style="display:flex;justify-content:space-between"><h3 style="color:#facc15;margin:0">📐 Z轴 v29.0｜'+sym+'</h3><button id="refreshBtn" style="background:#3b82f6;border:none;padding:4px 12px;border-radius:20px;color:white">🔄</button></div>'+
        '<div style="margin:12px 0">🎯 主导周期: '+topHtml+'</div>'+
        '<div style="display:flex;gap:20px;flex-wrap:wrap">'+
        '<div><div style="color:#facc15;font-size:0.7rem">📊 未来5日概率(价格斜率+频谱)</div>'+
        ...
        '<div style="margin-top:12px;font-size:0.6rem;color:#64748b;text-align:center">⚡ v29 | 价格斜率70%+频谱30% | 概率范围8%-92%</div></div>';
